...
public class VSRemoteObjectManager {
	...
	static private final VSRemoteObjectManager remoteObjectManager = new VSRemoteObjectManager();
	...
	public Remote exportObject(Remote object) throws IOException {
		...
		VSRemoteReference reference = null;
		ClassLoader ldr = object.getClass().getClassLoader();
		Class<?>[] intfs = object.getClass().getInterfaces();
		generierenThread();
		try {
			reference = new VSRemoteReference(Inet4Address.getLocalHost()
					.getHostAddress(), socket.getLocalPort(), objectID);
...
//						}
						...
						throw new RemoteException(
								"An error occured while invoking method.", exc);
					}
				}
			}
		} while ((c = c.getSuperclass()) != null);
		throw new RemoteException(
				"unable to invoke the methode in Impl, the methode is not found");
	}
