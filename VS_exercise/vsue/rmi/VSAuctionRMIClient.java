...
import vsue.rpc.VSAuctionEventHandler;
import vsue.rpc.VSAuctionEventType;
import vsue.rpc.VSAuctionException;
...
public class VSAuctionRMIClient implements VSAuctionEventHandler {
	...
	public void init(String registryHost, int registryPort) {
		...
		try {
			...
			proxy = VSRemoteObjectManager.getInstance().exportObject(this);
			VSObjectConnection connection = new VSObjectConnection(VSConnection client);
			//Registry registry = LocateRegistry.getRegistry(registryHost,registryPort);
			//service = (VSAuctionService) registry.lookup("service");
		} catch (IOException e) {
			// TODO Auto-generated catch block
			System.err.println("unable to set socket connection");
	...
	public void register(String auctionName, int duration, int startingPrice) {
		...
		try {
			((VSAuctionService) proxy).registerAuction(auction, duration, this);
